...
from sample_diffusion.diffusion_library.uni_pc import UniPC, NoiseScheduleVP
import math
...
class SamplerType(str, enum.Enum):
    ...
    def sample(self, model_fn, x_t, steps, callback, **sampler_args) -> torch.Tensor:
        if self == SamplerType.V_DDPM:
            ...
            if sampler_args.get(
                "is_reverse"
            ):  # HACK: Technically incorrect since DDIM implies eta > 0.0
                return vsampling.reverse_sample(
                    model_fn,
                    x_t,
                    steps,
            ...
            return vsampling.prk_sample(
                model_fn, x_t, steps, sampler_args.get("extra_args", {}), True, callback
            )
        ...
        elif self == SamplerType.UNIPC:
            # Compute sigma using the V_CRASH scheduler
            n = 20
            sigma = torch.sin(torch.linspace(1, 0, n + 1) * math.pi / 2) ** 2

            # Compute alphas and alphas_cumprod
            alphas = (1 - sigma**2) ** 0.5
            alphas_cumprod = torch.cumprod(alphas, dim=0)

            ns = NoiseScheduleVP("discrete", alphas_cumprod=alphas_cumprod)
            unipc = UniPC(model_fn, ns)
            return unipc.sample(x=x_t, steps=20)
        else:

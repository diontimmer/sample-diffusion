...
from sample_diffusion.util.util import tensor_slerp_2D, PosteriorSampling
...
class DDInference(InferenceBase):
    ...
    def generate(
        ...
        **kwargs
        ...
        if SamplerType.is_v_sampler(sampler):
            x_T = torch.randn(
                [batch_size, 2, self.model.chunk_size],
        ...
        **kwargs
        ...
        if SamplerType.is_v_sampler(sampler):
            ...
            x_T = alpha_T * audio_source + sigma_T * torch.randn(
                audio_source.shape, device=audio_source.device, generator=self.generator
            )
            model = self.model.model
        else:
    ...
    def generate_interpolation(
        ...
        **kwargs
        ...
        if self.optimize_memory_use and batch_size < 2:
            ...
            with self.offload_context(self.model.model):
                x_T_source = sampler.sample(
                    model, x_0_source, step_list.flip(0), callback, **sampler_args
                )

            with self.offload_context(self.model.model):
                x_T_target = sampler.sample(
                    model, x_0_target, step_list.flip(0), callback, **sampler_args
                )
            ...
            with self.offload_context(self.model.model):
                x_T = sampler.sample(
                    model, x_0, step_list.flip(0), callback, **sampler_args
                )
        ...
        x_Int = torch.empty(
            [batch_size, 2, self.model.chunk_size], device=self.device_accelerator
        )
        ...
        with self.offload_context(self.model.model):
            return sampler.sample(
                model, x_Int, step_list, callback, **sampler_args
            ).float()
    ...
    def generate_inpainting(
        ...
        **kwargs
        ...
        if method == "repaint":
            raise Exception(
                "Repaint currently not supported due to changed requirements"
            )
            ...
            if SamplerType.is_v_sampler(sampler):
                raise Exception(
                    "V-Sampler currently not supported for posterior guidance. Please choose a K-Sampler."
                )
            else:
                x_T = audio_source + step_list[0] * torch.randn(
                    [batch_size, 2, self.model.chunk_size],
    ...
    def generate_extension(
        ...
        **kwargs
        ...
        chunk = torch.cat(
            [
                audio_source[:, :, -half_chunk_size:],
                torch.zeros(
                    [batch_size, 2, half_chunk_size], device=self.device_accelerator
                ),
            ],
            dim=2,
        )
        ...
        if keep_start:
            return torch.cat([audio_source, output[:, :, -half_chunk_size:]], dim=2)
        else:

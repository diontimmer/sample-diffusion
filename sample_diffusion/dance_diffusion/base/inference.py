...
class InferenceBase:
    def __init__(
        ...
        self.generator = torch.Generator(
            device_accelerator
        )  # if (device_accelerator.type != 'mps') else torch.device('cpu'))
        self.rng_state = None

    def set_device_accelerator(self, device: torch.device = None):
        self.device_accelerator = device

    def get_device_accelerator(self) -> torch.device:
        return self.device_accelerator

    def set_model(self, model: ModelWrapperBase = None):
        self.model = model

    def get_model(self) -> ModelWrapperBase:
        return self.model

    def expand(self, tensor: torch.Tensor, expansion_map: list[int]) -> torch.Tensor:

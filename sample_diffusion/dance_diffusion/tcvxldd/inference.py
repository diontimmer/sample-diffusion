...
class TCVXLDDInference(InferenceBase):
    def __init__(
        self,
    ...
    def generate(
        ...
        **kwargs
        ...
        with self.offload_context(self.model.t5_embedder):
            text_embeddings = self.model.t5_embedder(sampler_args["text_condition"]).to(
            ...
            timing_embeddings = self.model.module.get_timing_embeddings(
                sampler_args["timestamp_embeddings"]
            )

            # move tensors in tuple timing_embeddings to device_accelerator
            timing_embeddings = tuple(
                timing_embedding.to(self.device_accelerator)
                for timing_embedding in timing_embeddings
            )
        ...
        sampler_args["extra_args"] = {
            ...
            "negative_embedding": negative_text_embeddings,
